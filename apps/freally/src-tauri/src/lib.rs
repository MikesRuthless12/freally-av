...
pub fn run() {
    ...
    // dev builds show what's going on. CLI / freallyctl wires its own
    ...
    // its own `freallykernel-scheduler` thread + current-thread runtime.
    ...
    builder
        ...
        .expect("error while running Freally Anti-Virus");
}
...
/// TS-side mirror of `freallykernel::updater::engine::EngineUpdateProgress`.
...
fn autostart_mechanism() -> &'static str {
    #[cfg(target_os = "linux")]
    {
        "~/.config/autostart/freally.desktop"
    }
    #[cfg(target_os = "macos")]
    {
...
fn init_state() -> Result<AppState, Box<dyn std::error::Error>> {
    ...
    let db_path = data_dir.join("freally.db");
    let conn = db::open(&db_path)?;
    let db = Arc::new(Mutex::new(conn));

    // Build the pipeline from whatever .bin feeds exist on disk. First
    // run has no feeds — that's fine; users add them via `freallyctl feed
...
fn build_database_channel(data_dir: &std::path::Path) -> DatabaseChannel {
    ...
    let nsrl_local = std::env::var("FREALLY_NSRL_LOCAL").unwrap_or_default();
