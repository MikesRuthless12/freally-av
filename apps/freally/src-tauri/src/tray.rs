...
//!
//! Implements FR-162. The tray icon is the Freally `M` glyph in one of
...
use freallykernel::realtime::shields::ShieldsActor;
use serde::Serialize;
use tauri::{
...
impl TrayIconState {
    ...
    pub fn tooltip(self) -> &'static str {
        match self {
            TrayIconState::Idle => "Freally Anti-Virus — idle",
            TrayIconState::Scanning => "Freally Anti-Virus — scanning",
            TrayIconState::ShieldsOff => "Freally Anti-Virus — Shields OFF",
            TrayIconState::UpdateAvailable => "Freally Anti-Virus — update available",
        }
    }
...
pub fn build_tray(app: &AppHandle) -> tauri::Result<()> {
    ...
    let tray = TrayIconBuilder::with_id("freally-tray")
        .icon(icon)
...
fn build_menu(app: &AppHandle) -> tauri::Result<Menu<Wry>> {
    ...
    let quit = MenuItem::with_id(app, "quit", "Quit Freally", true, None::<&str>)?;
...
fn handle_menu_event(app: &AppHandle, event: MenuEvent) {
    ...
    match id {
        ...
        // realistic session (the user will reboot or restart Freally
        // well before that).
...
mod tests {
    ...
    fn tooltips_include_brand_name() {
        for s in [
            ...
            assert!(s.tooltip().contains("Freally"), "{}", s.as_str());
        }
    }
