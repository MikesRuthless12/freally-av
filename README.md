...
Freally is **source-visible, All Rights Reserved**. See [`LICENSE.md`](LICENSE.md) for the operative text. In short:
...
**Phase 9 foundation 2026-05-27** — all Phase 9 Wave 1 + Wave 2 tasks landed: `daemon/freallyd-macos` real-time stack with `fsevents.rs` (primary NOTIFY-only surface), `esf_notify.rs` (opportunistic ES system extension — **no `endpoint-security.client` entitlement**, falls back to FSEvents on stock consumer macOS), `esf_failover.rs` (50 ms `(inode,mtime,size)` dedupe, prefers ESF, recovers gracefully on `ES_NEW_CLIENT_RESULT_ERR_NOT_PRIVILEGED`), `exemption_keychain.rs` + `crates/freallykernel/src/exempt/per_app.rs` (per-app exemptions stored in Keychain behind `BiometryCurrentSet | Or | DevicePasscode`; bundle ID + team ID composite key — pure path-based exemption rejected), `launchd.rs` + `Settings/MacExemptions.tsx` + `MacRealtimeHeartbeat.tsx` (atomic-write heartbeat JSON, green/amber/red chip), `crates/freallykernel/src/ipc/macesf.rs` (JSON-over-XPC `IpcFrame { NotifyEvent, Heartbeat, ShieldsPush, ActiveFindingsPush }` — **no Verdict variant; NOTIFY-only proven by `on_wire.contains("verdict")` test**), and `rules/honey.rs` (`proc_listpids` + `proc_pidinfo` process-tree walk + SIGSTOP, mac-side variant of TASK-142). Daemon binaries on Windows ship as Phase-8 scaffolds; full Windows ETW+AMSI+WDAC lands Phase 12.
...
Freally is built with Rust + Tauri v2 + pnpm + Solid.js. You will need:
...
```bash
pnpm -C apps/freally/frontend install --frozen-lockfile
...
2. **"This file came from another computer and might be blocked"** (Mark-of-the-Web):
   ...
   - Re-launch the MSI.
3. **Per-machine install** lands at `%ProgramFiles%\Freally Anti-Virus\` with a Start menu shortcut under `Freally Anti-Virus`. Uninstall via **Settings → Apps → Installed apps → Freally Anti-Virus**.
...
Freally pulls public threat intelligence from sources documented in [`THIRD-PARTY-DATA.md`](THIRD-PARTY-DATA.md). Headline:
...
Email `freallyone@gmail.com`. PGP key TBD. Coordinated disclosure with a 90-day window. See [`SECURITY.md`](SECURITY.md).

---

## Contact

- Project: github.com/MikesRuthless12/freally-av
- Maintainer email: `freallyone@gmail.com`
- Licensing inquiries: `freallyone@gmail.com`
- Security disclosures: `freallyone@gmail.com`
- Marketing site: `freally.com` (live from Phase 10)

---

*Freally Anti-Virus is a project of Mike Weaver. All Rights Reserved.*
