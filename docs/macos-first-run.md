# macOS first-run: opening an unsigned Freally bundle

Freally Anti-Virus ships **unsigned and unnotarized** on macOS. Per the
...
Double-clicking `Freally Anti-Virus.app` (or a freshly mounted
`.dmg`) the first time produces one of two dialogs depending on your
macOS version:

- macOS 13 (Ventura) and earlier:
  `"Freally Anti-Virus" can't be opened because it is from an unidentified developer.`
- macOS 14 (Sonoma) and later:
  `"Freally Anti-Virus" cannot be opened because Apple cannot check it for malicious software.`
...
1. Locate the `Freally Anti-Virus.app` bundle (either in `/Applications`
...
`"Freally Anti-Virus" was blocked from use because it is not from an
...
For independent verification of the bundle you downloaded, Freally
...
1. **Tauri Updater ed25519 signature.** Every release asset is signed
   ...
   listed in `apps/freally/src-tauri/tauri.conf.json` under
   `plugins.updater.pubkey`. Verify locally with `minisign`:
   ```sh
   minisign -V -p <our-pubkey> -m Freally\ Anti-Virus.app.tar.gz
   ```
...
(`apps/freally/src-tauri/entitlements.plist`):
