//! Freally Anti-Virus command-line interface (`freallyctl`).
...
#[command(
    name = "freallyctl",
    about = "Freally Anti-Virus command-line interface",
    version,
    long_about = None,
)]
struct Cli {
    /// Override the SQLite database path. Defaults to
    /// `<data_dir>/freally.db` per PRD § 3.
...
enum Commands {
    ...
    /// `freallyctl shields {on,off,status,pause <minutes>}`.
    Shields {
        #[command(subcommand)]
...
fn main() -> anyhow::Result<()> {
    ...
    // (e.g. `MYTH_LOG=info,freallykernel::walker=debug`) and falls back to
