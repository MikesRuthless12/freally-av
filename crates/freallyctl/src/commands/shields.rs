//! `freallyctl shields` (TASK-156).
//!
//! `freallyctl shields {on,off,status,pause <minutes>}` per FR-160.7. The
...
fn print_state(state: &freallykernel::realtime::shields::ShieldsState) {
    if state.enabled {
...
fn short(state: &freallykernel::realtime::shields::ShieldsState) -> &'static str {
    if state.enabled {
        "ON"
