...
/// produce.
///
/// - Windows: `%LOCALAPPDATA%\Freally\`
/// - macOS:   `~/Library/Application Support/Freally/`
/// - Linux:   `$XDG_DATA_HOME/freally/` or `~/.local/share/freally/`
pub fn default_data_dir() -> Result<PathBuf, DbError> {
    let base = directories::BaseDirs::new().ok_or(DbError::NoDataDir)?;
    let dir = if cfg!(target_os = "windows") {
        base.data_local_dir().join("Freally")
    } else if cfg!(target_os = "macos") {
        base.data_dir().join("Freally")
    } else {
        base.data_dir().join("freally")
    };
...
/// Default DB path: `<data_dir>/freally.db`.
pub fn default_db_path() -> Result<PathBuf, DbError> {
    Ok(default_data_dir()?.join("freally.db"))
}
