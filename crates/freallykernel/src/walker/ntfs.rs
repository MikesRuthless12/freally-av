...
mod windows_impl {
    ...
    pub fn walk(root: &Path, opts: WalkOpts) -> crossbeam_channel::Receiver<WalkEvent> {
        ...
        std::thread::Builder::new()
            .name("freallykernel/ntfs-walker".into())
...
mod macos_impl {
    ...
    pub fn walk(root: &Path, opts: WalkOpts) -> crossbeam_channel::Receiver<WalkEvent> {
        ...
        std::thread::Builder::new()
            .name("freallykernel/macos-walker".into())
            .spawn(move || match open_subscriber(&root) {
                Ok(sub) => run_bootstrap(sub, &root, &opts, &tx),
                Err(err) => {
                    tracing::info!(
...
mod linux_impl {
    ...
    pub fn walk(root: &Path, opts: WalkOpts) -> crossbeam_channel::Receiver<WalkEvent> {
        ...
        std::thread::Builder::new()
            .name("freallykernel/linux-walker".into())
            .spawn(move || match open_subscriber(&root) {
                Ok(sub) => run_bootstrap(sub, &root, &opts, &tx),
                Err(err) => {
                    tracing::info!(
...
mod tests {
    ...
    /// `cargo test --release -p freallykernel ntfs_walker_live -- --ignored --nocapture`.
