...
//!
//! Cursors live under `$XDG_DATA_HOME/freally/journal/<root_hash>.json`
//! (defaults to `~/.local/share/freally/journal/`).
//!
...
//! tamper vector lights up only once Phase 8's `freallyd-linux` daemon
...
impl WatchCursor {
    pub fn default_root() -> PathBuf {
        if let Some(xdg) = std::env::var_os("XDG_DATA_HOME") {
            return PathBuf::from(xdg).join("freally").join("journal");
        }
