...
//! lives here in freallykernel rather than in either daemon crate. The
//! `daemon/freallyd-{linux,windows}/` re-export from here.
//!
//! Pure parsing — no syscalls, no shell-outs. The actual `wsl.exe`
//! invocation lives in `daemon/freallyd-windows/src/wsl_bridge.rs::list_distros`.
