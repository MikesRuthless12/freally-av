...
//!  3. `logs/freally.log.YYYY-MM-DD` for the most recent N days
...
mod tests {
    ...
    fn build_bundle_produces_manifest_config_and_logs() {
        ...
        let earlier = log_dir.join("freally.log.2026-05-26");
        std::fs::write(
            &earlier,
            "{\"path\":\"/Users/me/keep\",\"level\":\"info\"}\nplain line\n",
        )
        .unwrap();
        let later = log_dir.join("freally.log.2026-05-27");
        ...
        let log_2027 = read_zip_entry(&out, "logs/freally.log.2026-05-27").unwrap();
        assert!(log_2027.contains("<REDACTED>"));
        assert!(!log_2027.contains("/secret"));
        let log_2026 = read_zip_entry(&out, "logs/freally.log.2026-05-26").unwrap();
    ...
    fn collect_recent_logs_truncates_to_count() {
        ...
        // its metadata).
        for i in 0..10 {
            let p = dir.path().join(format!("freally.log.2026-05-{i:02}"));
    ...
    fn bug_report_redacts_path_fields_from_logs() {
        let dir = tempdir().unwrap();
        let log = dir.path().join("freally.log.2026-05-27");
        std::fs::write(
            &log,
