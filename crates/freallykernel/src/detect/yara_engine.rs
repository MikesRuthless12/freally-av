...
//! Lets Freally ship public YARA rule packs (Neo23x0/
...
mod tests {
    ...
    fn matches_a_rule_against_real_bytes() {
        ...
        let rule_path = dir.path().join("freallykernel_smoke.yar");
        fs::write(
            &rule_path,
            r#"
            rule freallykernel_smoke_marker {
                meta:
                    description = "smoke test"
                    severity    = "medium"
                strings:
                    $a = "FREALLY_SMOKE_MARKER_42"
                condition:
                    $a
            }
            "#,
        ...
        match det.check(&ctx) {
            DetectorVerdict::Malicious {
                ...
                assert_eq!(rule_id, "yara:freallykernel_smoke_marker");
                assert_eq!(rule_source, "yara");
                assert_eq!(severity, Severity::Medium);
                let ev = evidence.expect("evidence present");
                assert!(ev.contains("rule=freallykernel_smoke_marker"), "got {ev}");
