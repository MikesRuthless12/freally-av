...
//! (`freallyctl scan` — TASK-017) and Tauri bridge (`scan_start` — TASK-028)
...
impl ScanEngine {
    ...
    fn scan_internal(
        ...
        let producer_spawn = std::thread::Builder::new()
            .name("freallykernel/scan-producer".into())
            .spawn(move || {
        ...
        std::thread::Builder::new()
            .name("freallykernel/scan-abort-watcher".into())
            .spawn(move || {
                loop {
        ...
        //
        // Off by default in release builds — set `FREALLY_HEARTBEAT=1`
        ...
        let heartbeat_enabled =
            cfg!(debug_assertions) || std::env::var_os("FREALLY_HEARTBEAT").is_some();
        ...
        if heartbeat_enabled {
            std::thread::Builder::new()
                .name("freallykernel/scan-heartbeat".into())
                .spawn(move || {
        ...
        std::thread::Builder::new()
            .name("freallykernel/scan-cursor-persister".into())
            .spawn(move || {
                loop {
                    std::thread::sleep(std::time::Duration::from_secs(5));
        ...
        let worker = tokio::task::spawn_blocking(move || {
            ...
            for worker_idx in 0..n_workers {
                ...
                let handle = std::thread::Builder::new()
                    .name(format!("freallykernel/scan-worker-{worker_idx}"))
                    .spawn(move || {
                        let ctx = WorkerCtx {
                            work_rx,
                            pause_flag,
