...
//! `freallyd-macos` binary and the engine-side `ui-bridge` consumer)
...
/// launchd as a LaunchAgent — see `packaging/macos/com.freally.freallyd.plist`).
/// The daemon `bind`s here; the engine-side connector `connect`s here.
/// Path is per-user under `~/Library/Application Support/Freally/`
/// because LaunchAgents have no `/run/<service>` equivalent.
pub const USER_SOCKET_PATH: &str = "~/Library/Application Support/Freally/freallyd.sock";

/// Canonical filename for the launchd heartbeat JSON. Re-exported
/// from both the daemon crate (`freallyd_macos::launchd::HEARTBEAT_FILENAME`)
