...
//! live under `<data_dir>/journal/<volume_serial>.json` (Mythodikal flavor;
...
impl VolumeCursor {
    ...
    /// Default cursor root: `<LOCALAPPDATA>\Mythodikal\journal`. Falls back
    /// to `<system_temp>\Mythodikal\journal` if `LOCALAPPDATA` is unset
    /// (service contexts that strip env vars).
    pub fn default_root() -> PathBuf {
        if let Some(local) = std::env::var_os("LOCALAPPDATA") {
            PathBuf::from(local).join("Mythodikal").join("journal")
        } else {
