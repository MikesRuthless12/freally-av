...
//! daily-rolling files at `<data_dir>/logs/mythodikal.log.YYYY-MM-DD`.
...
///
/// ```ignore
/// let _guard = mythkernel::logging::init(&mythkernel::logging::default_log_dir()?)?;
/// ```
pub fn init(log_dir: &Path) -> Result<WorkerGuard, EngineError> {
    std::fs::create_dir_all(log_dir)?;

    let file_appender = rolling::daily(log_dir, "mythodikal.log");
