//! `mythctl feed` (TASK-026 / FR-094).
//!
//! Drives the abuse.ch + NSRL feed updaters from `mythkernel::updater`.
...
pub struct UpdateArgs {
    ...
    #[arg(long, env = "MYTHODIKAL_ABUSECH_AUTH_KEY")]
...
async fn update(args: UpdateArgs) -> anyhow::Result<()> {
    ...
    match args.abusech_auth_key {
        ...
        _ => {
            eprintln!(
                "abuse.ch: skipped (no --abusech-auth-key / MYTHODIKAL_ABUSECH_AUTH_KEY). \
